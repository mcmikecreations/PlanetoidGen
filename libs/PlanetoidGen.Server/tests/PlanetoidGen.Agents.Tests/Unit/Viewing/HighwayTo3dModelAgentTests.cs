...
{
    public class HighwayTo3dModelAgentTests : BaseAgentTests
    {
        public HighwayTo3dModelAgentTests(ITestOutputHelper outputHelper) : base(outputHelper)
        {
        }
        ...
        public async Task GivenDefaultSettings_TestAssimpExport()
        {
            ...
            var filePath = Path.Combine(rootDir, "TestFiles/ExportedHighway.obj");

            var serviceProvider = ServiceProviderMock;

            var service = new HighwayTo3dModelService();
        ...
        private static async Task<Scene> GenerateScene(
            ...
            HighwayTo3dModelService service,
            AssimpGeometryConversionService geometryConversionService,
            IServiceProvider provider,
            CancellationToken token)
        {
            string geoText = "MULTILINESTRING((38.356038 48.9915433,38.3559003 48.9914306,38.3557816 48.9913146,38.3556676 48.991187))";
            var geometry = new WKTReader().Read(geoText);
            var entity = new HighwayEntity(42772501, "secondary", "asphalt", null, null, geometry);
