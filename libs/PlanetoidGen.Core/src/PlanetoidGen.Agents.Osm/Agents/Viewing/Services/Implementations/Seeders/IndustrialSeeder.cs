...
{
    internal class IndustrialSeeder : IBuildingSeeder
    {
        private const double LengthMax = 8.0;

        private readonly ILogger _logger;
        private readonly JsonSerializerOptions _serializerOptions;
        private readonly string _title;

        public IndustrialSeeder(ILogger logger, JsonSerializerOptions serializerOptions, string title)
        {
            _logger = logger;
            ...
            Litdex.Random.Random random)
        {
            ...
            if (levels == null)
            {
                var level = random.NextInt(0, 10);
                if (level < 6) levels = 1;
                else if (level < 9) levels = 2;
                else levels = 3;
            }
            else if (levels == 0)
            {
                ...
                foreach (var geom in multiPolygon)
                {
                    if (geom is Polygon polygon)
                    {
                        ...
                        if (ring.IsRing)
                        {
                            ...
                            for (var j = 0; j < levels.Value; ++j)
                            {
                                ...
                                for (var i = 0; i < sideCount; ++i)
                                {
                                    var parts = Enumerable
                                        ...
                                        .Select(y =>
                                        {
                                            int partRnd = random.NextInt(0, 2);
                            ...
                            {
                                var roofKindIndex = random.NextByte() % (isRoofComplex ? 2 : 3);
