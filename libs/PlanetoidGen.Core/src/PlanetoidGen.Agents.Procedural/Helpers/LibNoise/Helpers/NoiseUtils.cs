...
using PlanetoidGen.Agents.Procedural.Helpers.LibNoise.Constants;
using System;

namespace PlanetoidGen.Agents.Procedural.Helpers.LibNoise.Helpers
{
    public static class NoiseUtils
    {
        private const long X_NOISE_GEN = 1619;
        private const long Y_NOISE_GEN = 31337;
        private const long Z_NOISE_GEN = 6971;
        private const long SEED_NOISE_GEN = 1013;
        private const long SHIFT_NOISE_GEN = 8;
        private static readonly double[] _randomVectors = new double[256 * 4] {
        ...
        public static double GradientNoise3D(double fx, double fy, double fz, int ix, int iy, int iz, long seed)
        {
            ...
            var xvGradient = _randomVectors[vectorIndex << 2];
            var yvGradient = _randomVectors[(vectorIndex << 2) + 1];
            var zvGradient = _randomVectors[(vectorIndex << 2) + 2];
        ...
        public static void LatLonDegreesToXYZ(double lat, double lon, ref double x, ref double y, ref double z)
        {
            var r = Math.Cos(MathConst.DEG_TO_RAD * lat);
